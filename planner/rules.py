...
import rules
from planner.models import Rotation, PlanRequest, RotationRequest, RotationRequestResponse, RotationRequestForward, \
    RotationRequestForwardResponse

@rules.predicate
def is_owner(user, object):
    """
    Check if the user owns the passed object, which may be one of: Rotation, PlanRequest, RotationRequest,
     RotationRequestResponse, RotationRequestForward, RotationRequestForwardResponse.
    """
    if isinstance(object, Rotation) or isinstance(object, PlanRequest):
